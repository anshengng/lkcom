...
<template>
  <div class="flex items-center flex-col min-h-screen bg-gray-100 p-5">
    <section
      class="bg-white min-h-[250px] rounded-2xl flex flex-col justify-between px-2"
    >
      ...
      <div class="flex justify-between">
        <div class="flex gap-2 ml-5">
          ...
          <div class="flex flex-col">
            <div class="flex items-center">
              <div class="flex items-center">
                <span class="text-sm hover:text-red-500 cursor-pointer">{{
                  model?.user?.nickname || model?.user?.name
                }}</span>
            ...
            <div class="space-x-2 text-sm">
              ...
              <span>{{
                dayjs(model?.user?.createAt!).format("YY-MM-DD H:mm")
              }}</span>
            </div>
          </div>
        </div>
        <div v-show="model.userId === user.userInfo.id">
          <el-button
            ...
            >修改</el-button
          >
          <el-button type="danger" size="default" @click="" plain
            >删除</el-button
          >
        </div>
      </div>
    ...
    <section
      class="rounded-2xl p-2 gap-2 flex flex-col justify-between px-2 mt-10 bg-white min-h-[200px]"
    >
      <CommentItem
        v-for="comment in model?.comment"
        :key="comment.id"
        :comment="comment"
        :postmaster="model?.id"
        @reply="reply"
      ...
      <el-button type="primary" @click="addComment(model.id!)" class="w-52 m-2"
        >发表评论</el-button
      >
    </section>
  </div>
